...
// Resolve Persian label mappings
...
// Resolve subcategory name (assumes model has a method like getNameById)
...
<div class="max-w-5xl mx-auto p-6 space-y-6" dir="rtl">
  ...
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-bold text-gray-800">لیست تقاضاهای من</h2>
    <a href="<?= route('ietannounce.create') ?>" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-medium transition duration-150 ease-in-out transform hover:-translate-y-0.5 flex items-center gap-1">
...
<?php $announces=$data['my_demands']?>
        ...
        <div class="bg-white p-5 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 border-l-4 <?= $announce['supply_demand'] === 'supply' ? 'border-blue-500' : 'border-green-500' ?>">
          ...
          <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div class="flex-1">
                ...
                <div>
                  ...
                  <div class="flex flex-wrap gap-2 text-sm">
                    <span class="px-2 py-1 bg-blue-100 text-blue-800 font-medium rounded-full">
                      <?= htmlspecialchars($announce['main_sub_cate_name'] ?? '---') ?>
                    </span>
                    <span class="px-2 py-1 <?= $announce['supply_demand'] === 'supply' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' ?> font-medium rounded-full"><?= $sd_label ?></span>
                    <span class="px-2 py-1 bg-purple-100 text-purple-800 font-medium rounded-full"><?= $gs_label ?></span>
                  </div>
              ...
              <p class="text-gray-600 mt-3 leading-relaxed"><?= mb_strimwidth(strip_tags($announce['description']), 0, 160, '...') ?></p>
            </div>
          </div>
        </div>
      <?php endforeach; ?>
  ...
  <?php else: ?>
    <div class="bg-white p-8 rounded-lg shadow-sm text-center">
      ...
      <p class="mt-1 text-gray-500">می‌توانید اولین آگهی خود را ایجاد کنید</p>
      <div class="mt-6">
        <a href="<?= route('ietannounce.create') ?>" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-medium transition-colors duration-200 inline-flex items-center">
