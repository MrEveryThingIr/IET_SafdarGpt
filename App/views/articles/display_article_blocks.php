...
function render_article_block(array $block): string
{
    ...
    $editTriggerId = "editTrigger_{$blockId}";
    $editModalId = "editTarget_{$blockId}";
    $deleteUrl = route('ietarticles.block.delete', ['id' => $blockId]);

$editDeleteButtons = <<<HTML
<div class="absolute top-0 right-0 mt-2 mr-2 flex gap-2 z-30 bg-white bg-opacity-90 p-1 rounded">
    ...
    <form action="{$deleteUrl}" method="post">
        ...
        <button class="px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 transition">حذف</button>
    </form>
</div>
HTML;
    ...
    switch ($type) {
      ...
      case 'heading':
        ...
        <div class="flex flex-col gap-1 {$class}">
            <{$headingTag} class="mt-6 mb-2 font-bold text-xl">{$content}</{$headingTag}>
        </div>
        HTML;
            break;
...
HTML;
        ...
        case 'audio':
            ...
            $output = <<<HTML
<{$type} controls class="w-full rounded shadow {$class}">
    <source src="{$src}">
...
HTML;
        ...
        case 'list':
            ...
            $listItems = implode('', array_map(fn($item) => '<li class="ml-4 list-inside">' . htmlspecialchars($item) . '</li>', $items));
...
<div class="my-4 {$class}">
    <h3 class="font-semibold text-lg text-blue-800">❓ {$content}</h3>
...
$editModal = render_edit_modal($block);

return <<<HTML
<div class="relative group border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
    {$editDeleteButtons}
    {$output}
    <div id="{$editModalId}" class="modal hidden fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
        {$editModal}
    </div>
</div>
HTML;
