    ...
    public function requestRecoverPassword(): void
{
    try {
        ...
        $resetLink = rtrim(BASE_URL, '/') . route('recoverpassword.resetform', ['token' => $token]);
...
<body style='font-family: Arial, sans-serif; line-height: 1.6;'>
    ...
    <p>If you did not request this, please ignore this email.</p>
        ...
        if (!$emailService->send($email, 'Password Recovery', $emailBody)) {
...
public function showResetForm(string $token): void
{
    ...
    if (!$resetRecord || strtotime($resetRecord['expiry']) < time()) {
        $_SESSION['error'] = 'Invalid or expired token.';
        redirect(route('auth.login'));
    }

    echo $this->render('auth/recoverpassword/reset_password', ['token' => $token]);
}
